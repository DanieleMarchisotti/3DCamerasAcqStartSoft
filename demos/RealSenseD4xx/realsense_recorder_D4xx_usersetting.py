...
if __name__ == "__main__":
    ...
    depth_profiles,color_profiles,synch_depth_profiles_idx, synch_color_profiles_idx= writeCameraModes.writeRGBDModes(pipeline)
    Mode=int(input("Enter the camera stream index mode: "))
    # folderPathToSave=input("Enter folder path to save: ")
    # realFPS=int(input("Enter FPS (lower than the camera one): "))
    config = rs.config()
    ...
    clipping_distance_in_meters = 1*10
    ...
    stringToWrite= writeParameters.to_file(profile_depth, profile_color, meas_unit_selected, depth_sensor,
